...
* File:: fsShell.c
*
* Description:: Main driver for file assignment.
*
...
int cmd_touch (int argcnt, char *argvec[])
		...
		// printf("(SHELL) O_WRONLY = %d\n", O_WRONLY);
		// printf("(SHELL) O_CREAT = %d\n", O_CREAT);
		// printf("(SHELL) O_WRONLY | O_CREAT = %d\n", (O_WRONLY | O_CREAT));
...
int cmd_mv (int argcnt, char *argvec[])
	...
	if (argcnt != 3){
		...
		// Will call parse path for both source and destination.
		int srcRetVal = parsePath(argvec[1], &srcInfo);
		int destRetVal = parsePath(argvec[2], &destInfo);

		// load the source in the 'source' directory entry pointer.
		DirectoryEntry * source = &srcInfo.parent[srcInfo.deNum];

		// Will fail if the path for either the source or destination is
		// not valid.
		if(destRetVal == -1 || srcRetVal == -1){
			printf("(MOVE) Invalid destination path.\n");
			return -1;
		}

		// Will fail if the source does not exist in the source parent.
		if(srcInfo.deNum == -1){
			printf("mv: %s No such file or directory\n", argvec[1]);
			return -1;
		}

		// If the destination doesn't exist it will either make a new file or a 
		// directory based on the source that was provided by the user.
		if(destInfo.deNum == -1){
			// if a source is specified but destination doesn't exist.
			if(source->isDirectory){
				// create a new directory and add the data from the source to dest
				// remove the source from its original location.
				int newDirLocation = fs_mkdir(argvec[2], 0);
				int dirIndex = findInDirectory(destInfo.lastElement, destInfo.parent);

				strcpy(destInfo.parent[dirIndex].dirName, destInfo.lastElement);
				destInfo.parent[dirIndex].location = source->location;
				destInfo.parent[dirIndex].size = source->size;
				destInfo.parent[dirIndex].isDirectory = source->isDirectory;
				destInfo.parent[dirIndex].dateCreated = source->dateCreated;
				destInfo.parent[dirIndex].dateModified = source->dateModified;				

				// Set everything inside the source to either null or 0 
				// which removes the entry from the source.
				strcpy(source->dirName, "");
				source->dateCreated = 0;
				source->dateModified = 0;
				source->isDirectory = 0;
				source->location = 0;
				source->size = 0;

				// Save both the source and the destinations.
				saveDir(destInfo.parent);
				saveDir(srcInfo.parent);

			} else{

				int newFileLocation = findSpaceInDir(destInfo.parent);


				strcpy(destInfo.parent[newFileLocation].dirName, destInfo.lastElement);
				destInfo.parent[newFileLocation].location = source->location;
				destInfo.parent[newFileLocation].size = source->size;
				destInfo.parent[newFileLocation].isDirectory = source->isDirectory;
				destInfo.parent[newFileLocation].dateCreated = source->dateCreated;
				destInfo.parent[newFileLocation].dateModified = source->dateModified;				
			...
			// if the source is specified and the destination exists.
		}else {
			// change the pointer from the og parent to the new location.
			// NOTE: doesn't change actual disk location.
			

			if(source->isDirectory){
				if(findSpaceInDir(&destInfo.parent[destInfo.deNum]) < 0){
					printf("[MOVE FAILED] The directory is full.\n");
				}
				// transfer the pointer from the source to the destination
				// remove the source from its original location.
				if(!destInfo.parent[destInfo.deNum].isDirectory){
					printf("[ERROR] Cannot move directory to a file\n");
					return -1;
				}
				DirectoryEntry * loadedDir = loadDirectory(&destInfo.parent[destInfo.deNum]);
				int newLocation = findSpaceInDir(loadedDir);

				strcpy(loadedDir[newLocation].dirName, srcInfo.lastElement);
				loadedDir[newLocation].location = source->location;
				loadedDir[newLocation].size = source->size;
				loadedDir[newLocation].isDirectory = source->isDirectory;
				loadedDir[newLocation].dateCreated = source->dateCreated;
				loadedDir[newLocation].dateModified = source->dateModified;				
				...
				// replace the original file with the source and remove the original.
				DirectoryEntry * loadedDir;
				int newFileLocation;
				
				if(destInfo.parent[destInfo.deNum].isDirectory){
					loadedDir = loadDirectory(&destInfo.parent[destInfo.deNum]);
					newFileLocation = findSpaceInDir(loadedDir);
				} else{
					loadedDir = destInfo.parent;
					newFileLocation = destInfo.deNum;
				}

				if(!loadedDir[newFileLocation].isDirectory && loadedDir[newFileLocation].size > 0){
					printf("[MOVE FAILED] Cannot move to a file that is not empty.\n");
					return -1;
				}

				

				if(findSpaceInDir(loadedDir) < 0){
					printf("[MOVE FAILED] The directory is full.\n");
				}


				strcpy(loadedDir[newFileLocation].dirName, srcInfo.lastElement);
				loadedDir[newFileLocation].location = srcInfo.parent[srcInfo.deNum].location;
				loadedDir[newFileLocation].size = srcInfo.parent[srcInfo.deNum].size;
				loadedDir[newFileLocation].isDirectory = srcInfo.parent[srcInfo.deNum].isDirectory;
				loadedDir[newFileLocation].dateCreated = srcInfo.parent[srcInfo.deNum].dateCreated;
				loadedDir[newFileLocation].dateModified = srcInfo.parent[srcInfo.deNum].dateModified;				
